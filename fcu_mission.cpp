...
void execute_mission_001(void){
 ...
 // if(enable_track){
  ...
  if(enable_track){
    ...
    // ====================== 核心修改：圆心(1.2,1.2)，半径0.8m ======================
    // bridge1 (001号机) 基准绕圆轨迹
    px  = 0.8 * cosf(theta) + 1.2;    // 半径0.8，圆心x=1.2
    py  = 0.8 * sinf(theta) + 1.2;    // 半径0.8，圆心y=1.2
    px1 = px; py1 = py; pz1 = 0.6;     // 高度固定0.6m

    // 002号机：角度偏移 60°
    px2 = 0.8 * cosf(theta + M_PI*2/6) + 1.2;
    py2 = 0.8 * sinf(theta + M_PI*2/6) + 1.2;
    pz2 = 0.6;

    // 003号机：角度偏移 120°
    px3 = 0.8 * cosf(theta + M_PI*4/6) + 1.2;
    py3 = 0.8 * sinf(theta + M_PI*4/6) + 1.2;
    pz3 = 0.6;

    // 004号机：角度偏移 180°
    px4 = 0.8 * cosf(theta + M_PI*6/6) + 1.2;
    py4 = 0.8 * sinf(theta + M_PI*6/6) + 1.2;
    pz4 = 0.6;

    // 005号机：角度偏移 240°
    px5 = 0.8 * cosf(theta + M_PI*8/6) + 1.2;
    py5 = 0.8 * sinf(theta + M_PI*8/6) + 1.2;
    pz5 = 0.6;

    // 006号机：角度偏移 300°
    px6 = 0.8 * cosf(theta + M_PI*10/6) + 1.2;
    py6 = 0.8 * sinf(theta + M_PI*10/6) + 1.2;
    pz6 = 0.6;
    // ===========================================================================
  }else{
      switch(enable_pos){
